...
export const getUsers = asyncHandler(async (req, res, next) => {
  ...
  !users ? next(new ErrorResponse(404, USER_NOT_FOUND)) : SuccessResponse(200, users, req, res);
});

export const getUserByID = asyncHandler(async (req, res, next) => {
  const user = await User.findById(req.params.userID).select('-__v');

  !user ? next(new ErrorResponse(404, USER_NOT_FOUND)) : SuccessResponse(200, user, req, res);
});
...
export const deleteUser = asyncHandler(async (req, res, next) => {
  const deletedUser = await User.findByIdAndDelete(req.params.userID).select('-__v');
